...
import type { ExpiryAlertItem, AnomalyAlertItem, ActiveAlertsReport } from '@/shared/domain/alerts'
...
export async function getActiveAlerts(
  ...
  try {
    const [inventoriesSnap, thresholdDays] = await Promise.all([
      ...
      providedThreshold !== undefined ? Promise.resolve(providedThreshold) : fetchAlertThreshold(associationId),
    ])
